...
import searchengine.db.sql.FileSql;
...
public class FileIndexRepository {
    ...
    public Map<String, String> findIndexedModifiedTimesUnderRoot(String rootPath) throws SQLException {
        ...
        try (PreparedStatement ps = connection.prepareStatement(FileSql.FIND_INDEXED_FILES_UNDER_ROOT)) {
    ...
    public void deleteByPath(String path) throws SQLException {
        try (PreparedStatement deleteFtsPs = connection.prepareStatement(FileSql.DELETE_FTS_BY_PATH)) {
            deleteFtsPs.setString(1, path);
            deleteFtsPs.executeUpdate();
        }

        try (PreparedStatement deleteFilePs = connection.prepareStatement(FileSql.DELETE_FILE_BY_PATH)) {
    ...
    private void replaceIndexedContent(IndexedFileData fileData) throws SQLException {
        try (PreparedStatement deletePs = connection.prepareStatement(FileSql.DELETE_FTS_BY_PATH)) {
        ...
        try (PreparedStatement insertPs = connection.prepareStatement(FileSql.INSERT_FTS_ROW)) {
