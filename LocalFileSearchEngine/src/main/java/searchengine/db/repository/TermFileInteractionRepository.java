...
import searchengine.db.sql.TermFileInteractionSql;
...
public class TermFileInteractionRepository {
    ...
    public void recordTermFileInteraction(String term, String path) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(TermFileInteractionSql.UPSERT_TERM_FILE_INTERACTION)) {
    ...
    private void keepTopFilesForTerm(String term) {
        try (PreparedStatement ps = connection.prepareStatement(TermFileInteractionSql.KEEP_TOP_FILES_FOR_TERM)) {
            ps.setString(1, term);
            ps.setString(2, term);
