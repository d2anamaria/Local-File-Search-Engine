...
public class DatabaseMaintenanceService {
    ...
    private void deleteOldResultInteractions(Connection connection, double days) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement(ResultInteractionSql.DELETE_OLD_RESULT_INTERACTIONS)) {
            ps.setDouble(1, days);
            ps.executeUpdate();
    ...
    private void deleteOldSearchHistory(Connection connection, double days) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement(SearchHistorySql.DELETE_OLD_SEARCH_HISTORY)) {
            ps.setDouble(1, days);
    ...
    private void deleteOrphanResultInteractions(Connection connection) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement(ResultInteractionSql.DELETE_ORPHAN_RESULT_INTERACTIONS)) {
            int deleted = ps.executeUpdate();
    ...
    private void deleteOldTermFileInteractions(Connection connection, double days) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement(TermFileInteractionSql.DELETE_OLD_TERM_FILE_INTERACTIONS)) {
            ps.setDouble(1, days);
            int deleted = ps.executeUpdate();
    ...
    private void deleteOrphanTermFileInteractions(Connection connection) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement(TermFileInteractionSql.DELETE_ORPHAN_TERM_FILE_INTERACTIONS)) {
            int deleted = ps.executeUpdate();
